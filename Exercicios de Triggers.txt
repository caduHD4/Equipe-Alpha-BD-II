...
INSERT INTO recebimento (especie, desconto ,sem_desconto,venda_id ,ten_caixa_id) VALUES ("avista",850.00,950.00,1,1);
...
BEGIN
        ...
        SELECT estoque INTO estocagem FROM produto WHERE id = NEW.produto_id;
...
BEGIN
    ...
    SELECT especie INTO tipo FROM recebimento WHERE id = NEW.recebimento_id;
    
	IF tipo = "avista" THEN
		UPDATE iten_caixa SET valor_novo = valor_novo + valor + NEW.desconto WHERE id = NEW.iten_caixa_id;
    ELSE
		UPDATE iten_caixa SET valor_novo = valor_novo + valor + NEW.sem_desconto WHERE id = NEW.iten_caixa_id;
	END IF;
END;
//
DELIMITER ;
